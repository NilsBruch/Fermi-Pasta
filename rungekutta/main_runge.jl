...
function main()
    ...
    t = 300      #time to solve the equation for
    Δt = 0.0001    #timestep width
    nstep=Int(t/Δt)  #Numer of timesteps

    Δt_per_f=3000  #Timesteps per output/frame in gif


    filename="rungekutta/data/spectrum_harmonic" #Output filename
    #Starting conditions
    u0=zeros(2*N) #u0[1:N]: x0[1:N],   u0[N+1:2N]=v0
    x=range(0, N-1; step=1)
    u0[1:N]=sin.(16*π/(N-1)*x)

    cfg = config(N, α, β, u0) # Initialize configuration
